...
Import note
-----------
Routers are imported with relative-style paths (``from routes.cameras``) so
this module works both when run from the project root (``python -m uvicorn
apps.backend.main:app``) and inside the Docker image where the build context
is ``apps/backend/`` and only that directory is on sys.path.
"""
